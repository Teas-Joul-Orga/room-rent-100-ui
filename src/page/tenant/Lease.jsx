...
export default function TenantLease() {
  ...
  const totalRentPaid = (lease.payments || []).filter(p => p.type === "rent").reduce((s, p) => s + Number(p.amount_paid), 0);
  const totalContractMonths = Math.max(1, dayjs(lease.end_date).diff(dayjs(lease.start_date), 'month'));
  const totalContractValue = Number(lease.rent_amount) * totalContractMonths;
          ...
          <Table variant="simple" size="md">
            ...
            <Tbody>
              {(lease.payments || []).length === 0 ? (
                <Tr>
                  <Td colSpan={4} textAlign="center" py={8} color={mutedText}>
                    No payment history found.
                  </Td>
                </Tr>
              ) : (
                [...lease.payments].sort((a,b) => dayjs(b.payment_date).diff(dayjs(a.payment_date))).map((payment) => (
                  <Tr key={payment.id}>
                    <Td fontSize="sm" fontWeight="bold" color={textColor}>
                      {fmtDate(payment.payment_date)}
                    </Td>
                    <Td fontSize="sm" textTransform="uppercase" fontWeight="bold" color={mutedText}>
                      {payment.payment_method}
                    </Td>
                    <Td fontSize="sm" fontWeight="black" color="green.500">

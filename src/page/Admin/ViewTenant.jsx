...
export default function ViewTenant() {
  ...
  const handleUpdatePassword = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/v1/admin/tenants/${id}/change-password`, {
        method: "POST",
        headers: {
           ...
           <Box overflowX="auto">
             <Table variant="simple">
               <Thead bg={tableHeaderBg} boxShadow="sm" position="relative" zIndex={1}>
                 <Tr>
                   <Th fontSize="xs">Room</Th>
                   <Th fontSize="xs">Start Date</Th>
                   <Th fontSize="xs">End Date</Th>
                   <Th isNumeric fontSize="xs">Rent Amount</Th>
                 ...
                 {tenant.leases && tenant.leases.length > 0 ? (
                   tenant.leases.map((lease) => (
                     <Tr key={lease.id} _hover={{ bg: hoverBg }}>
                       <Td color={textColor}>
                         <Text fontWeight="bold">{lease.room?.name || `Room ${lease.room_id}`}</Text>
                       </Td>
                       <Td>
                         <HStack spacing={2}>
                           <Icon as={LuCalendar} color="blue.400" />
                           <Text fontSize="sm" fontWeight="medium" color={textColor}>{fmtDate(lease.start_date)}</Text>
                         </HStack>
                       </Td>
                       <Td>
                         <HStack spacing={2}>
                           <Icon as={LuCalendar} color="orange.400" />
                           <Text fontSize="sm" fontWeight="medium" color={textColor}>{fmtDate(lease.end_date)}</Text>
                         </HStack>
                       </Td>
                       <Td isNumeric fontWeight="black" color={textColor}>
                   ...
                   <Tr>
                     <Td colSpan={5} textAlign="center" py={8} color={mutedTextColor} fontStyle="italic">
